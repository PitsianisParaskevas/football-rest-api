...
import type { TransformedStatistics } from "./types/types/results.type";
import type { TransformedLineups } from "./types/types/lineup.type";
import type { TransformedIncidents } from "./types/types/incidetns.type";
import type { TransformedShotmap } from "./types/types/shotmap.type";
import type {
  AvgPositionEntry,
  TransformedAvgPosition,
} from "./types/types/avgPosition.type";
import type {
  ExtractedPlayerBasicInfo,
  PlayerBasicInfo,
  PlayerHeatmap,
} from "./types/types/basicPlayerInfo.type";
  ...
  const processTeam = (team: typeof lineups.home, side: "home" | "away") => {
    ...
    for (const player of team.statisticsPlayer) {
      ...
      const calculateMinutesPlayed = (): number | null => {
        if (subIn && subOut) {
          console.log("player came in and was subbed out again", player);
          return subOut.time - subIn.time;
        } else if (subOut) {
          console.log("starter who was subbed off", player);
          return subOut.time;
        } else if (subIn) {
          console.log("sub who came in and played to end", player);
          return 90 - subIn.time;
        } else if (rating !== null) {
          console.log("full 90-minute player", player);
          return 90;
        } else {
          console.log("bench player who didn't play", player);
          return 0;
        }
      };
