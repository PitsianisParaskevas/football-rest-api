...
import { getPlayerGoal } from "@/functions/getPlayerGoal";
...
router.get(
  "/getPlayerGoal/:playerCustId",
  asyncHandler(async (req, res) => {
    const playerCustId = Number(req.params.playerCustId);
    if (!Number.isFinite(playerCustId)) {
      res.status(400).json({ message: "Invalid player_cust_id" });
      return;
    }
    const data = await getPlayerGoal(playerCustId);
    res.json(data);
  })
);
